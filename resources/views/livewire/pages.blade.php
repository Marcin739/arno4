<div class="p-6">
    <div class="flex items-center justify-end px-4 py-3 text-right sm:px-6">
        <x-jet-button wire:click="createShowModal" class="bg-green-500" >
            {{ __('Create') }}
        </x-jet-button>
    </div>
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 text-sm whitespace-nowrap">
                                            <a class="text-indigo-600 hover:text-indigo-900" target="_blank"
                                                href="{{ URL::to('/'.$item->slug) }}"> {{ $item->slug }} </a> 
                                        </td>
                                        <td class="px-6 py-4 text-sm whitespace-nowrap">{!! \Illuminate\Support\Str::limit($item->content, 50, ' ...')  !!}</td>
                                        <td class="px-6 py-4 text-right text-sm">
                                            <x-jet-button wire:click="updateShowModal({{ $item->id }})"  class="bg-indigo-500">
                                                {{ __('Update') }}
                                            </x-jet-button>
    ...
    <x-jet-dialog-modal wire:model="modalFormVisible">
        ...
        <x-slot name="content">
            <div class="mt-4">
                ...
                <x-jet-input id="title" class="block mt-1 w-full rounded-md border border-r-1 border-gray-300" 
                type="title" wire:model.debounce.800ms="title" />
                @error('title') <span class="error">{{ $message }}</span>@enderror
            </div>
            <div class="mt-4">
                <x-jet-label for="title" value="{{ __('Slug') }}" />
                <div class="mt-1 flex rounded-md shadow-sm border border-r-1 border-gray-300">
                    <span class="inline-flex items-center px-3 rounded-1-md  
                    bg-gray-200 text-gray-500 text-sm"> http://localhost:8000/ </span>
                    <input wire:model="slug" class="form-input flex-1 px-6 block w-full rounded-none rounded-r-md 
                    transition duration-150 ease-in-out sm:text-sm sm:leading-5" placeholder="url-slug">
                </div>
                ...
                <div class="rounded-md shadow-sm">
                    <div class="mt-1 bg-white">
                        <div class="block mt-1 w-full">
                            <textarea class="form-control w-full rounded-md border border-r-1 
                                border-gray-300" wire:model.debounce.4000ms="content"></textarea>
                        </div>
                    </div>
                </div>
        ...
        <x-slot name="footer">
            ...
            @if ($modelId)
                <x-jet-button class="ml-2" wire:click="update" wire:loading.attr="disabled"  class="bg-yellow-700">
                    {{ __('Save') }}
                </x-jet-button>                
            @else
    ...
    <x-jet-dialog-modal wire:model="modalConfirmDeleteVisible">
        ...
        <x-slot name="footer">
            ...
            <x-jet-danger-button class="ml-2" wire:click="delete" wire:loading.attr="disabled"  class="bg-blue-500">
                {{ __('Delete Page') }}
            </x-jet-danger-button>
        </x-slot>
    </x-jet-dialog-modal>
